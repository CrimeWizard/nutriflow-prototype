...
export function poolForSlot(slot: MealSlotId, goal: Goal, area: string): MealPick[] {
  ...
  if (slot === 'lunch') {
    const lunchRecipes = recipeList.filter((r) => !r.tags.includes('breakfast'));
    if (goal === 'cut') {
      return [
        ...meals.filter((m) =>
    ...
    if (goal === 'bulk') {
      return [
        ...meals.filter((m) => m.protein >= 32 || m.tags.some((t) => HEAVY_TAGS.has(t))),
        ...lunchRecipes.filter((r) =>
          r.tags.includes('high-protein') || r.tags.includes('meal-prep'),
        ),
      ];
    }
    return [
