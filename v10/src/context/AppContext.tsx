...
function loadOnboardingComplete(): boolean {
  try { return localStorage.getItem('nf-v10-onboarded') === 'true'; } catch { return false; }
}

function loadProfile(): UserProfile {
  try {
    const raw = localStorage.getItem('nf-v10-profile');
...
function loadSupermarketId(): string {
  try {
    const id = localStorage.getItem('nf-v10-supermarket');
    if (id) return id;
  } catch { /* empty */ }
  return 'sm-carrefour';
}
...
function loadOrderHistory(): Order[] {
  try {
    const raw = localStorage.getItem('nf-v10-orders');
    if (raw) {
...
function saveOrderHistory(orders: Order[]) {
  try { localStorage.setItem('nf-v10-orders', JSON.stringify(orders)); } catch { /* empty */ }
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setSelectedSupermarketId = useCallback((id: string) => {
    ...
    try { localStorage.setItem('nf-v10-supermarket', id); } catch { /* empty */ }
  }, []);
  ...
  const setProfile = useCallback((patch: Partial<UserProfile>) => {
    setProfileState((prev) => {
      ...
      try { localStorage.setItem('nf-v10-profile', JSON.stringify(next)); } catch { /* empty */ }
      return next;
    });
  }, []);
  ...
  const completeOnboarding = useCallback(() => {
    try { localStorage.setItem('nf-v10-onboarded', 'true'); } catch { /* empty */ }
    navStackRef.current = [];
    setScreenState('home');
    setTab('home');
  ...
  const resetOnboarding = useCallback(() => {
    try {
      localStorage.removeItem('nf-v10-onboarded');
      localStorage.removeItem('nf-v10-profile');
      localStorage.removeItem('nf-v10-supermarket');
      localStorage.removeItem('nf-v10-orders');
      localStorage.removeItem('nf-v10-favorites');
    } catch { /* empty */ }
