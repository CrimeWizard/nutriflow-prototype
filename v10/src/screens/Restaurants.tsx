...
export function Restaurants() {
  const { profile, openRestaurant, setScreen } = useApp();
  ...
  return (
    <div className="scroll fade-in">
      <ScreenHeader
        title="Eat out"
        subtitle={`Healthy restaurants delivering to ${profile.area}`}
      />
      ...
      {list.length === 0 ? (
        <div className="empty">
          ...
          {!query && filter === 'all' && (
            <button type="button" className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => setScreen('groceries')}>
              Browse groceries
            </button>
          )}
        </div>
