...
function loadOnboardingComplete(): boolean {
  try { return localStorage.getItem('nf-v9-onboarded') === 'true'; } catch { return false; }
}

function loadProfile(): UserProfile {
  try {
    const raw = localStorage.getItem('nf-v9-profile');
...
function loadSupermarketId(): string {
  try {
    const id = localStorage.getItem('nf-v9-supermarket');
    if (id) return id;
  } catch { /* empty */ }
  return 'sm-carrefour';
}
...
function loadOrderHistory(): Order[] {
  try {
    const raw = localStorage.getItem('nf-v9-orders');
    if (raw) {
...
function saveOrderHistory(orders: Order[]) {
  try { localStorage.setItem('nf-v9-orders', JSON.stringify(orders)); } catch { /* empty */ }
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setProfile = useCallback((patch: Partial<UserProfile>) => {
    setProfileState((prev) => {
      ...
      try { localStorage.setItem('nf-v9-profile', JSON.stringify(next)); } catch { /* empty */ }
      return next;
    });
  }, []);
  ...
  const completeOnboarding = useCallback(() => {
    try { localStorage.setItem('nf-v9-onboarded', 'true'); } catch { /* empty */ }
    setScreen('home');
    setTab('home');
  ...
  const resetOnboarding = useCallback(() => {
    try {
      localStorage.removeItem('nf-v9-onboarded');
      localStorage.removeItem('nf-v9-profile');
      localStorage.removeItem('nf-v9-supermarket');
      localStorage.removeItem('nf-v9-orders');
