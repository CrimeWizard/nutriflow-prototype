...
function loadOnboardingComplete(): boolean {
  try { return localStorage.getItem('nf-v5-onboarded') === 'true'; } catch { return false; }
}

function loadProfile(): UserProfile {
  try {
    const raw = localStorage.getItem('nf-v5-profile');
...
function loadSupermarketId(): string {
  try {
    const id = localStorage.getItem('nf-v5-supermarket');
    if (id) return id;
  } catch { /* empty */ }
  return 'sm-carrefour';
}
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const weeklyPlan = useMemo(
    ...
    [profile.goal, profile.gymDays, profile.gymTime, profile.area],
  );
  ...
  const setSelectedSupermarketId = useCallback((id: string) => {
    ...
    try { localStorage.setItem('nf-v5-supermarket', id); } catch { /* empty */ }
  }, []);

  const setProfile = useCallback((patch: Partial<UserProfile>) => {
    setProfileState((prev) => {
      const next = { ...prev, ...patch };
      try { localStorage.setItem('nf-v5-profile', JSON.stringify(next)); } catch { /* empty */ }
      return next;
    });
  }, []);
  ...
  const completeOnboarding = useCallback(() => {
    try { localStorage.setItem('nf-v5-onboarded', 'true'); } catch { /* empty */ }
    setScreen('home');
    setTab('home');
  ...
  const resetOnboarding = useCallback(() => {
    try {
      localStorage.removeItem('nf-v5-onboarded');
      localStorage.removeItem('nf-v5-profile');
      localStorage.removeItem('nf-v5-supermarket');
    } catch { /* empty */ }
  ...
  const plannedMealToCartItems = useCallback((meal: PlannedMeal): CartItem[] => {
