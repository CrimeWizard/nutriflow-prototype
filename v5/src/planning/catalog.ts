...
export function restaurantsInArea(area: string): Restaurant[] {
  const local = restaurants.filter((r) => r.area === area);
  return local.length > 0 ? local : restaurants;
}

function flattenRestaurantMeals(area: string): RestaurantPick[] {
  const inAreaIds = new Set(restaurantsInArea(area).map((r) => r.id));
  const picks: RestaurantPick[] = [];
  restaurants.forEach((r) => {
    r.meals.forEach((m) => {
      picks.push(mealToPick(r, m, inAreaIds.has(r.id)));
    });
  });
  return picks;
}
